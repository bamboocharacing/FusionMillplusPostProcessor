...
// Extract as global declare
         ...
         */
        function setMachineConfiguration(
         ...
         */
        function getMultiAxisMoveLength(
            x: Number,
        ): MoveLength;
...
OPTMIZE_HEADS: Map tool tip position for machine axes for heads only. 
         ...
         */
        function optimizeMachineAngles2(
...
OPTMIZE_HEADS: Map tool tip position for machine axes for heads only. 
         ...
         */
        function optimizeMachineAnglesByMachine(
            machine: MachineConfiguration,
        ): void;
         ...
         * @since r41185 
         */
        function isSectionSpecialCycle(
            uri: String
        ): Boolean;
         ...
         * @since r41185 
         */
        function setSectionSpecialCycle(
            uri: String,
        ): void;
         ...
         * @since r50217 
         */
        function getFramePositionFromPolar(
            x: Number,
        ): Vector;
         ...
         */
        function activateAutoPolarMode(): void;
         ...
         * @since r45635 
         */
        function setCurrentPositionAndDirection(
            posDir: VectorPair
        ): void;
         ...
         * @since 41546 
         */
        function setCascadingPath(): void;
         ...
         */
        function getRedirectionBuffer2(
            clear: Boolean
        ): String;
         ...
         */
        function registerPostProcessing(
            path: String
        ): void;
         ...
         */
        function getMachineConfigurationByName(
            name: String
        ): MachineConfiguration;
         ...
         */
        function getSectionFramePosition(
         ...
         */
        function getSectionFrameDirection(
         ...
         * @since r45909 
         */
        function getCurrentABC(): Vector;

        /**
         * Returns the current tool axis.
         * @since r45909 
         */
        function getCurrentToolAxis(): Vector;

        /**
         * Sets the current tool axis.
         * @since r45909 
         */
        function setCurrentToolAxis(
         ...
         * @since r42066 
         */
        function setCurrentSpindleSpeed(
            spindleSpeed: Number
        ): void;
...
The machine angles set by this function will be cleared by a call to {@link cancelRotation()} or {@link cancelTransformation()} 
         ...
         * @since r45843 
         */
        function isDepositionOperation(
            section: Section
        ): Boolean;
         ...
         * @since r42045 
         */
        function getNumberOfSyncGroups(
            channel: Integer
        ): Integer;
...
TOOL_TABLE_NUM_COLS 
         ...
         * @since r45991 
         */
        function getCircularOffset(): Vector;
         ...
         * @since r45991 
         */
        function getCircularArcLength(): Number;
         ...
         */
        function getActiveFeedMode(): Integer;
...
e.g.
         * @since r50353 
         */
        function setSupportedFeatures(): void;
         ...
         * @since 41664 
         */
        function registerTerminationHandler(): void;
         ...
         */
        function isSameDirection(): Boolean;
         ...
         * @since r42272 
         */
        function getProgramNameAsString(
            charLimit: Integer
        ): String;
         ...
         */
        function onExpandedSpindleSpeed(
            spindleSpeed: Number
        ): void;
         ...
         * @param specifiers The specifiers. 
         */
        function createMachineConfiguration(
            specifiers: Map<any, any> // This generic type have been defaulted to any types.
        ): MachineConfiguration;
         ...
         * @param machine The machine configuration. 
         */
        function getMachineConfigurationAsText(
            machine: MachineConfiguration
        ): String;
         ...
         * @param format The desired format.
         */
        function createIncrementalVariable(
            specifiers: Map<any, any>, // This generic type have been defaulted to any types.
        ): IncrementalVariable;
         ...
         */
        function createReferenceVariable(
            specifiers: Map<any, any>, // This generic type have been defaulted to any types.
        ): ReferenceVariable;
         ...
         */
        function repositionToCycleClearance(
         ...
         * @since r45400 
         */
        function onReturnFromSafeRetractPosition(
            x: Number,
        ): void;
        ...
        const outputUnit: Integer;

        const currentSection: Section;

        const highFeedMapping: Integer;

        const highFeedrate: Number;

        const lineNumber: Integer;

        const initialCyclePosition: Vector;

        const abortOnDeprecation: Boolean;
        ...
        const programNameIsInteger: Boolean;

        const debugMode: Boolean;

        const preventPost: Boolean;
        ...
        const certificationLevel: Integer;

        const revision: Integer;

        const minimumRevision: Integer;

        const deprecated: Boolean;

        const capabilities: Integer;

        const tolerance: Number;

        const mapWorkOrigin: Boolean;

        const mapToWCS: Boolean;

        const allowMachineChangeOnSection: Boolean;

        const minimumChordLength: Number;

        const minimumCircularRadius: Number;

        const maximumCircularRadius: Number;

        const minimumCircularSweep: Number;

        const maximumCircularSweep: Number;

        const maxRotaryDeltaAngle: Number;

        const circularInputTolerance: Number;

        const circularMergeTolerance: Number;

        const allowHelicalMoves: Boolean;

        const allowSpiralMoves: Boolean;

        const allowedCircularPlanes: Integer;

        const circularOutputAccuracy: Integer;

        const linearOutputAccuracy: Integer;

        const rotaryOutputAccuracy: Integer;

        const bufferRotaryMoves: Boolean;

        const controlSupportsTWP: Boolean;

        const eulerConvention: Integer;

        const workPlaneCalculationMethod: Integer;

        const allowFeedPerRevolutionDrilling: Integer;

        const machineParameters: MachineParameters;
        ...
        /* Circular XY plane. */
        const PLANE_XY: Integer;

        /* Circular XZ plane. Deprecated use PLANE_ZX instead. */
        const PLANE_XZ: Integer;

        /* Circular ZX plane. */
        const PLANE_ZX: Integer;

        /* Circular YZ plane. */
        const PLANE_YZ: Integer;
        ...
        /* YZ-plane. */
        const TOOL_AXIS_X: Integer;

        /* ZX-plane. */
        const TOOL_AXIS_Y: Integer;

        /* XY-plane. */
        const TOOL_AXIS_Z: Integer;

        /* Center radius compensation. */
        const RADIUS_COMPENSATION_OFF: Integer;

        /* Left radius compensation. */
        const RADIUS_COMPENSATION_LEFT: Integer;

        /* Right radius compensation. */
        const RADIUS_COMPENSATION_RIGHT: Integer;

        /* Don't linearize moves around multi-axis singularities.  More... */
        const SINGULARITY_LINEARIZE_OFF: Integer;

        const SINGULARITY_LINEARIZE_LINEAR: Integer;

        /* Keep rotary axes in line during multi-axis singularity linearization.  More... */
        const SINGULARITY_LINEARIZE_ROTARY: Integer;

        /* Coolant disabled. */
        const COOLANT_DISABLED: Integer;

        /* Flood coolant mode. */
        const COOLANT_FLOOD: Integer;

        /* Mist coolant mode. */
        const COOLANT_MIST: Integer;

        /* Coolant through tool mode. Deprecated use COOLANT_THROUGH_TOOL instead. */
        const COOLANT_TOOL: Integer;

        /* Coolant through tool mode. */
        const COOLANT_THROUGH_TOOL: Integer;

        /* Air mode. */
        const COOLANT_AIR: Integer;

        /* Air through tool mode. */
        const COOLANT_AIR_THROUGH_TOOL: Integer;

        /* Suction mode. */
        const COOLANT_SUCTION: Integer;

        /* Flood and mist coolant mode. */
        const COOLANT_FLOOD_MIST: Integer;

        /* Flood and through tool coolant mode. */
        const COOLANT_FLOOD_THROUGH_TOOL: Integer;

        /* Unspecified material. */
        const MATERIAL_UNSPECIFIED: Integer;

        /* High-speed steel material. */
        const MATERIAL_HSS: Integer;

        /* TI coated material. */
        const MATERIAL_TI_COATED: Integer;

        /* Carbide material. */
        const MATERIAL_CARBIDE: Integer;

        /* Ceramics material. */
        const MATERIAL_CERAMICS: Integer;

        /* Unspecified tool. */
        const TOOL_UNSPECIFIED: Integer;

        /* Drill. */
        const TOOL_DRILL: Integer;

        /* Center drill. */
        const TOOL_DRILL_CENTER: Integer;

        /* Spot drill. */
        const TOOL_DRILL_SPOT: Integer;

        /* Block drill. */
        const TOOL_DRILL_BLOCK: Integer;

        /* Flat end-mill. */
        const TOOL_MILLING_END_FLAT: Integer;

        /* Ball end-mill. */
        const TOOL_MILLING_END_BALL: Integer;

        /* Bullnose mill. */
        const TOOL_MILLING_END_BULLNOSE: Integer;

        /* Chamfer mill. */
        const TOOL_MILLING_CHAMFER: Integer;

        /* Face mill. */
        const TOOL_MILLING_FACE: Integer;

        /* Slot mill. */
        const TOOL_MILLING_SLOT: Integer;

        /* Radius mill. */
        const TOOL_MILLING_RADIUS: Integer;

        /* Dovetail mill. */
        const TOOL_MILLING_DOVETAIL: Integer;

        /* Tapered mill. */
        const TOOL_MILLING_TAPERED: Integer;

        /* Lollipop mill. */
        const TOOL_MILLING_LOLLIPOP: Integer;

        /* Right tap tool. */
        const TOOL_TAP_RIGHT_HAND: Integer;

        /* Left tap tool. */
        const TOOL_TAP_LEFT_HAND: Integer;

        /* Reamer tool. */
        const TOOL_REAMER: Integer;

        /* Boring bar tool. */
        const TOOL_BORING_BAR: Integer;

        /* Counterbore tool. */
        const TOOL_COUNTER_BORE: Integer;

        /* Countersink tool. */
        const TOOL_COUNTER_SINK: Integer;

        /* Holder. */
        const TOOL_HOLDER_ONLY: Integer;

        /* General turning tool. */
        const TOOL_TURNING_GENERAL: Integer;

        /* Thread turning tool. */
        const TOOL_TURNING_THREADING: Integer;

        /* Groove turning tool. */
        const TOOL_TURNING_GROOVING: Integer;

        /* Boring turning tool. */
        const TOOL_TURNING_BORING: Integer;

        /* Custom turning tool. */
        const TOOL_TURNING_CUSTOM: Integer;

        /* Probe. */
        const TOOL_PROBE: Integer;

        /* Wire. */
        const TOOL_WIRE: Integer;

        /* Water jet. */
        const TOOL_WATER_JET: Integer;

        /* Laser cutter. */
        const TOOL_LASER_CUTTER: Integer;

        /* Welder. */
        const TOOL_WELDER: Integer;

        /* Grinder. */
        const TOOL_GRINDER: Integer;

        /* Form mill. */
        const TOOL_MILLING_FORM: Integer;

        /* Plasma cutter. */
        const TOOL_PLASMA_CUTTER: Integer;

        /* Marker tool. */
        const TOOL_MARKER: Integer;

        /* Thread mill. */
        const TOOL_MILLING_THREAD: Integer;

        /* Turning tool compensation. */
        const TOOL_COMPENSATION_INSERT_CENTER: Integer;

        /* Turning tool compensation. */
        const TOOL_COMPENSATION_TIP: Integer;

        /* Turning tool compensation. */
        const TOOL_COMPENSATION_TIP_CENTER: Integer;

        /* Turning tool compensation. */
        const TOOL_COMPENSATION_TIP_TANGENT: Integer;

        /* Has parameter flag. */
        const HAS_PARAMETER: Integer;

        /* Has rapid flag. */
        const HAS_RAPID: Integer;

        /* Has linear flag. */
        const HAS_LINEAR: Integer;

        /* Has dwell flag. */
        const HAS_DWELL: Integer;

        /* Has circular flag. */
        const HAS_CIRCULAR: Integer;

        /* Has cycle flag. */
        const HAS_CYCLE: Integer;

        /* Has well-known command flag. */
        const HAS_WELL_KNOWN_COMMAND: Integer;

        /* Has comment flag. */
        const HAS_COMMENT: Integer;

        /* Invalid record type. */
        const RECORD_INVALID: Integer;

        /* Well-known command (onCommand). */
        const RECORD_WELL_KNOWN_COMMAND: Integer;

        /* Machine configuration change (onMachine). */
        const RECORD_MACHINE_COMMAND: Integer;

        /* Spindle speed (onSpindlSpeed) */
        const RECORD_SPINDLE_SPEED: Integer;

        /* Parameter (onParameter). */
        const RECORD_PARAMETER: Integer;

        /* Linear motion (onLinear, onRapid, onCyclePoint). */
        const RECORD_LINEAR: Integer;

        /* Linear 5-axis motion (onLinear5D, onRapid5D). */
        const RECORD_LINEAR_5D: Integer;

        /* Linear 5-axis motion (onLinear5D, onRapid5D). */
        const RECORD_LINEAR_ZXN: Integer;

        /* Linear motion with extrude (onLinearExtrude). */
        const RECORD_LINEAR_EXTRUDE: Integer;

        /* Circular motion (onCircular). */
        const RECORD_CIRCULAR: Integer;

        /* Dwell (onDwell). */
        const RECORD_DWELL: Integer;

        /* Cycle (onCycle). */
        const RECORD_CYCLE: Integer;

        /* End of cycle (onCycleEnd). */
        const RECORD_CYCLE_OFF: Integer;

        /* Comment (onComment, onManualNC). */
        const RECORD_COMMENT: Integer;

        /* Comment (onComment, onManualNC). */
        const RECORD_WIDE_COMMENT: Integer;

        /* Pass through (onPassThrough, onManualNC) */
        const RECORD_PASS_THROUGH: Integer;

        /* Pass through (onPassThrough, onManualNC) */
        const RECORD_WIDE_PASS_THROUGH: Integer;

        /* Start of operation (onSection, onSectionSpecialCycle). */
        const RECORD_OPERATION: Integer;

        /* End of operation (onSectionEnd, onSectionEndSpecialCycle). */
        const RECORD_OPERATION_END: Integer;

        /* Circular motion with extrude (onCircularExtrude). */
        const RECORD_CIRCULAR_EXTRUDE: Integer;

        /* Invalid (well-known command). */
        const COMMAND_INVALID: Integer;

        /* Program stop (well-known command M00). */
        const COMMAND_STOP: Integer;

        /* Optional program stop (well-known command M01). */
        const COMMAND_OPTIONAL_STOP: Integer;

        /* Program end (well-known command M02). */
        const COMMAND_END: Integer;

        /* Clockwise spindle direction (well-known command M03). */
        const COMMAND_SPINDLE_CLOCKWISE: Integer;

        /* Counterclockwise spidle direction (well-known command M04). */
        const COMMAND_SPINDLE_COUNTERCLOCKWISE: Integer;

        const COMMAND_START_SPINDLE: Integer;

        /* Spindle stop (well-known command M05). */
        const COMMAND_STOP_SPINDLE: Integer;

        const COMMAND_ORIENTATE_SPINDLE: Integer;

        /* Tool change (M06). */
        const COMMAND_LOAD_TOOL: Integer;

        /* Coolant on (M08). */
        const COMMAND_COOLANT_ON: Integer;

        /* Coolant off (M09). */
        const COMMAND_COOLANT_OFF: Integer;

        /* Activate speed-feed synchronization (well-known command). */
        const COMMAND_ACTIVATE_SPEED_FEED_SYNCHRONIZATION: Integer;

        /* Deactivate speed-feed synchronization (well-known command). */
        const COMMAND_DEACTIVATE_SPEED_FEED_SYNCHRONIZATION: Integer;

        /* Locks the 4th and 5th axes. This command is optional. */
        const COMMAND_LOCK_MULTI_AXIS: Integer;

        /* Unlocks the 4th and 5th axes. This command is optional. */
        const COMMAND_UNLOCK_MULTI_AXIS: Integer;

        /* Exact stop. This command is optional. */
        const COMMAND_EXACT_STOP: Integer;

        /* Close chip transport. */
        const COMMAND_START_CHIP_TRANSPORT: Integer;

        /* Stop chip transport. */
        const COMMAND_STOP_CHIP_TRANSPORT: Integer;

        /* Open primary door. */
        const COMMAND_OPEN_DOOR: Integer;

        /* Close primary door. */
        const COMMAND_CLOSE_DOOR: Integer;

        /* Break control. */
        const COMMAND_BREAK_CONTROL: Integer;

        /* Measure tool. */
        const COMMAND_TOOL_MEASURE: Integer;

        /* Run calibration cycle. */
        const COMMAND_CALIBRATE: Integer;

        /* Verify part/tool/machine integrity. */
        const COMMAND_VERIFY: Integer;

        /* Run cleaning cycle. */
        const COMMAND_CLEAN: Integer;

        /* Alarm. */
        const COMMAND_ALARM: Integer;

        /* Alert. */
        const COMMAND_ALERT: Integer;

        /* Change pallet. */
        const COMMAND_CHANGE_PALLET: Integer;

        /* Power on. */
        const COMMAND_POWER_ON: Integer;

        /* Power off. */
        const COMMAND_POWER_OFF: Integer;

        /* Open main chuck.  More... */
        const COMMAND_MAIN_CHUCK_OPEN: Integer;

        /* Close main chuck.  More... */
        const COMMAND_MAIN_CHUCK_CLOSE: Integer;

        /* Open secondary chuck.  More... */
        const COMMAND_SECONDARY_CHUCK_OPEN: Integer;

        /* Close secondary chuck.  More... */
        const COMMAND_SECONDARY_CHUCK_CLOSE: Integer;

        /* Activate spindle synchronization.  More... */
        const COMMAND_SECONDARY_SPINDLE_SYNCHRONIZATION_ACTIVATE: Integer;

        /* Deactivate spindle synchronization.  More... */
        const COMMAND_SECONDARY_SPINDLE_SYNCHRONIZATION_DEACTIVATE: Integer;

        /* Sync channels. */
        const COMMAND_SYNC_CHANNELS: Integer;

        /* Probe on. */
        const COMMAND_PROBE_ON: Integer;

        /* Probe off. */
        const COMMAND_PROBE_OFF: Integer;

        /* Rapid movement type. */
        const MOVEMENT_RAPID: Integer;

        /* Lead-in movement type. */
        const MOVEMENT_LEAD_IN: Integer;

        /* Cutting movement type. */
        const MOVEMENT_CUTTING: Integer;

        /* Lead-out movement type. */
        const MOVEMENT_LEAD_OUT: Integer;

        /* Transition linking movement type. */
        const MOVEMENT_LINK_TRANSITION: Integer;

        /* Additive bridging movement type. */
        const MOVEMENT_BRIDGING: Integer;

        /* Direction linking movement type. */
        const MOVEMENT_LINK_DIRECT: Integer;

        /* Helical ramp movement type. */
        const MOVEMENT_RAMP_HELIX: Integer;

        /* Helical jet pierce movement type. */
        const MOVEMENT_PIERCE_CIRCULAR: Integer;

        /* Profile ramp movement type. */
        const MOVEMENT_RAMP_PROFILE: Integer;

        /* Profile jet pierce movement type. */
        const MOVEMENT_PIERCE_PROFILE: Integer;

        /* Zig-zag ramp movement type. */
        const MOVEMENT_RAMP_ZIG_ZAG: Integer;

        /* Linear jet pierce movement type. */
        const MOVEMENT_PIERCE_LINEAR: Integer;

        /* Ramp movement type. */
        const MOVEMENT_RAMP: Integer;

        /* Plunge movement type. */
        const MOVEMENT_PLUNGE: Integer;

        /* Plunge jet pierce movement type. */
        const MOVEMENT_PIERCE: Integer;

        /* Predrill movement type. */
        const MOVEMENT_PREDRILL: Integer;

        /* Extended movement type. */
        const MOVEMENT_EXTENDED: Integer;

        /* Reduced cutting feed movement type. */
        const MOVEMENT_REDUCED: Integer;

        /* Finish cutting movement type. */
        const MOVEMENT_FINISH_CUTTING: Integer;

        /* High feed movement type. */
        const MOVEMENT_HIGH_FEED: Integer;

        /* Depositing movement type. */
        const MOVEMENT_DEPOSITING: Integer;

        /* Connection between toolpaths. */
        const MOVEMENT_CONNECTION: Integer;

        /* The drill has broken through material. */
        const MOVEMENT_DRILL_BREAKTHROUGH: Integer;

        /* Positioning the drill in the pilot for gun drilling. */
        const MOVEMENT_GUN_DRILL_POSITIONING: Integer;

        /* Do not map rapid travesal to high feed. */
        const HIGH_FEED_NO_MAPPING: Integer;

        /* Map rapid travesal along more than one axis to high feed. */
        const HIGH_FEED_MAP_MULTI: Integer;

        const HIGH_FEED_MAP_XY_Z: Integer;

        /* Map all rapid travesals to high feed. */
        const HIGH_FEED_MAP_ANY: Integer;

        const tool: Tool;

        const spindleAxis: Integer;

        const feedrate: Number;

        const activeFeedMode: Integer;
